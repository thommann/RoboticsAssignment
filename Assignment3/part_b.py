...
def region_growing(_img, _seed):
    ...
    while (intensity_difference < region_threshold) & (region_size < image_size):
        ...
        for i in range(4):
            ...
            x_new = _seed[0] + neighbors[i][0]
            y_new = _seed[1] + neighbors[i][1]
            ...
            if check_inside:
                if segmented_img[x_new, y_new] == 0:
                    ...
                    neighbor_intensity_list.append(_img[x_new, y_new])
        ...
        segmented_img[_seed[0], _seed[1]] = 255
        region_size += 1
        ...
        _seed = neighbor_points_list[index]
...
file_names = glob.glob(f'object_images/*.jpg')
filename = file_names[0]
img = cv2.imread(filename, 0)
resized = cv2.resize(img, (256, 256))
seed = 0, 0
objects = region_growing(resized, seed)
